 ...
 * Optionally summarize the produced Markdown via `pi` (default: opencode-go/deepseek-v4-flash).
 *
 ...
 *
 * Usage:
 *   node to-markdown.mjs <url-or-path> [--out <file>] [--tmp] [--summary [prompt]] [--prompt <prompt>] [--model <model>]
 *
 * Examples:
 ...
 *   node to-markdown.mjs ./spec.pdf --summary --model opencode-go/deepseek-v4-flash
 */
...
function usageAndExit(code = 1) {
  console.error('Usage: node to-markdown.mjs <url-or-path> [--out <file>] [--tmp] [--summary [prompt]] [--prompt <prompt>] [--model <model>]');
  process.exit(code);
}
...
let summaryModel = 'opencode-go/deepseek-v4-flash';
...
for (let i = 0; i < argv.length; i++) {
  ...
  if (a === '--model') {
    const val = argv[i + 1] ?? '';
    if (!val || isFlag(val)) {
      console.error('Expected a value after --model');
      process.exit(1);
    }
    summaryModel = val;
    i++;
...
function summarizeWithPi(markdown, { mdPathForNote = null, extraPrompt = null, model = 'opencode-go/deepseek-v4-flash' } = {}) {
...
async function main() {
  ...
  if (doSummary) {
    const summary = summarizeWithPi(md, { mdPathForNote: tmpMdPath ?? outPath, extraPrompt: summaryPrompt, model: summaryModel });
