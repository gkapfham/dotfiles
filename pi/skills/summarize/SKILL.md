...
### Convert + summarize with deepseek-v4-flash (pass context!)
...
Or pick a different model with `--model`:

```bash
node to-markdown.mjs ./spec.pdf --summary --model opencode-go/qwen3.7-plus \
  --prompt "Extract API endpoints and auth details."
```
...
3) run `pi --model <model>` (no-tools, no-session) to summarize using your extra prompt

The default model is `opencode-go/deepseek-v4-flash`. Override with `--model provider/model`.
