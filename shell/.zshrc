...
# Display all of the possible Zellij layouts with fzf and preview with bat;
# make sure that when no layout has yet been selected that there
# is a message that indicates that the user should select a layout
zp() {
  local layout
  layout=$( /run/current-system/sw/bin/ls ~/.config/zellij/layouts/*.kdl 2>/dev/null | xargs -n 1 basename | sed 's/\.kdl$//' | \
    fzf --query="$1" --select-1 --exit-0 --cycle --no-separator --prompt " " \
    --preview '[[ -n {} ]] && bat --style=numbers --color=always ~/.config/zellij/layouts/{}.kdl || echo "󰮊 Select a Zellij Layout"')
  if [[ -n "$layout" ]]; then
    zellij -n ~/.config/zellij/layouts/"$layout".kdl -s "$layout"
  fi
}
alias zpp="zp"

# Combined Zellij session and layout picker
# Shows existing sessions (running and resurrectable) and available layouts
# Use Ctrl+n and Ctrl+p for navigation (fzf default)
zs() {
  local choice type name
  
  # Build selection list: sessions first, then a separator, then layouts
  choice=$(
    {
      # List running sessions
      zellij list-sessions 2>/dev/null | sed 's/\x1b\[[0-9;]*m//g' | awk '{print "SESSION: " $1}' 
      
      # List resurrectable sessions
      zellij list-sessions --dead 2>/dev/null | sed 's/\x1b\[[0-9;]*m//g' | awk '{print "RESURRECT: " $1}'
      
      # Add separator
      echo "─────────────────────"
      
      # List layouts
      /run/current-system/sw/bin/ls ~/.config/zellij/layouts/*.kdl 2>/dev/null | xargs -n 1 basename | sed 's/\.kdl$//' | awk '{print "LAYOUT: " $1}'
    } | fzf --query="$1" --cycle --no-separator --prompt "󱂬 " \
      --header="Enter: Select | Ctrl+n/p: Navigate" \
      --preview='
        line={}
        if [[ $line == SESSION:* ]] || [[ $line == RESURRECT:* ]]; then
          session=$(echo $line | cut -d" " -f2)
          echo " Session: $session"
          echo ""
          zellij action dump-layout -s "$session" 2>/dev/null || echo "Session information not available"
        elif [[ $line == LAYOUT:* ]]; then
          layout=$(echo $line | cut -d" " -f2)
          bat --style=numbers --color=always ~/.config/zellij/layouts/"$layout".kdl 2>/dev/null
        fi
      ' \
  ...
  if [[ -n "$choice" ]]; then
    ...
    case "$type" in
      ...
      RESURRECT)
        ...
        ;;
      LAYOUT)
        zellij -n ~/.config/zellij/layouts/"$name".kdl -s "$name"
        ;;
    esac
  fi
}
